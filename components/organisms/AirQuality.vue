<template>
  <div class="card p-5">
    <h2 class="section-label mb-4">Air Quality</h2>

    <!-- Loading -->
    <div v-if="isLoading" class="flex flex-col gap-2">
      <div class="skeleton-card h-8 rounded-[8px]" />
      <div class="skeleton-card h-6 w-2/3 rounded-[8px]" />
    </div>
    ...
    <template v-else>
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-base font-body font-semibold text-ink">{{ meta.label }}</p>
            <p class="text-xs text-ink/45 font-body mt-0.5">{{ aqiDescription }}</p>
          </div>
        </div>
      </div>
