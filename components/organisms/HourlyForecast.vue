<template>
  <div class="card p-5">
    <h2 class="section-label mb-4">Hourly Forecast</h2>

    <div v-if="loading" class="flex gap-2 overflow-x-auto no-scrollbar pb-1">
      <div v-for="i in 8" :key="i" class="skeleton-card w-16 h-24 rounded-[8px] shrink-0" />
    </div>
    ...
    <div v-else-if="hours.length">
      ...
      <div class="relative">
        ...
        <div class="absolute right-0 top-0 bottom-0 w-10 bg-gradient-to-l from-canvas-pure to-transparent pointer-events-none rounded-r-card" />
      </div>
        ...
        >
          <div class="flex items-start justify-between mb-3">
            <div>
              ...
              <p class="text-sm font-body text-ink/70 mt-0.5 capitalize">{{ selectedHour.condition }}</p>
            </div>
